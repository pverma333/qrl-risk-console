...
if run:
    ...
    with ecol1:
        if scenarios:
            csv_out = pd.DataFrame(scenarios).to_csv(index=False).encode("utf-8")
            st.download_button(
                label="Download Scenarios CSV",
                data=csv_out,
                file_name=f"var_scenarios_{symbol}_{trade_date_str}.csv",
                mime="text/csv",
            )
    ...
    with ecol2:
        ...
        st.download_button(
            ...
            file_name=f"var_manifest_{symbol}_{trade_date_str}.json",
